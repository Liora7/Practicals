...
class EdBuffer {
    ...
    /** Transpose the characters to the left and right of the cursor. */
    def transpose(pos1: Int, pos2: Int){
        val ch1 = text.charAt(pos1)
        val ch2 = text.charAt(pos2)
        insert(pos2 + 1, ch1)
        deleteChar(pos1)
        setModified()
    }
    
    /** Copy the text between pos1 and pos2. */
    def copy(pos1: Int, pos2: Int){
      _buf = text.subSequence(pos1, pos2).toString
    }
    
    /** Cut the text between pos1 and pos1. */
    def cut(pos1: Int, pos2: Int){
       copy(pos1, pos2)
       deleteRange(pos1, pos2 - pos1)
    }
    
    /** Paste the text in _buf at current pos. */
    def paste(pos: Int){
      insert(pos, _buf)
      point = pos + _buf.length
    }
    
    /** Search the text for prompt and set pos to end of next occurrence */
    def search(pos: Int, prompt: String): Boolean = {
      ...
      while (s != pos - 1 && !found) {
        if (e <= text.length){
          if (text.subSequence(s, e).toString == prompt) found = true
        }
        if (!found){
          if (s == text.length - 1) {
            s = 0
