...
class UndoableEditor extends Editor with UndoHistory {
    ...
    override def insertCommand(ch: Char) {
        ...
        lastChange = new AmalgInsertion(ed.point-1, ch, 0)
    }
    
    /** Record of insertion that can be amalgamated with adjacent, similar changes */
    class AmalgInsertion(val pos: Int, ch: Char, mov: Int) extends Change {
    ...
    /** Command: move and record change */
    override def moveCommand(dir: Int) {
    ...
    /** Command: Transpose chars and record change */
    override def transposeCommand() = {
      var p = ed.point
      super.transposeCommand
      if (ed.point >= ed.length) {
        lastChange = new Change {
          def undo() { ed.transpose(ed.length - 2, ed.length - 1) }
          def redo() { ed.transpose(ed.length - 2, ed.length - 1) }
        }
      }
      else {
        lastChange = new Change {
          def undo() { ed.transpose(p-1, p) }
          def redo() { ed.transpose(p-1, p) }
        }
      }
    }
    
    /** Command: Cut and record change */
    override def cutCommand() {
      super.cutCommand
      var clip = ed.buf
      lastChange = new Change{
        def undo() { ed.insert(ed.mark, clip) }
        def redo() { ed.deleteRange(ed.mark, ed.buf.length) }
      }
    }
    
    /** Command: Paste and record change */
    override def pasteCommand() {
      var p = ed.point
      var clip = ed.buf
      super.pasteCommand
      lastChange = new Change{
        def undo() { ed.deleteRange(p, ed.buf.length) }
        def redo() { ed.insert(p, clip) }
      }
    }
