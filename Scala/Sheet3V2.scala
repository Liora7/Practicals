...
class ServerTotalStack[T]{
  ...
  private val getElem = OneMany[Option[T]]
  private val putElem = ManyOne[T]
  server.fork
  ...
  * @return Some(x) where x is the value popped, or None if the stack is empty. */
  ...
  private def server: PROC = proc{
    ...
    serve(
        putElem =?=> {x => stack.push(x)}
      | getElem =!=> {if (stack.isEmpty) None else Some(stack.pop)}
    )
  }

  def shutdown = {
    putElem.close; getElem.close
  }
...
class TreeSync(n: Int){
  ...
  def sync(me: Int) = {
    ...
    if(child1 < n) down(child1)!() // wait for sync request from both children
...
class PrefixSums(n: Int, a: Array[Int]){
  ...
  private def summer(me: Int) = proc("summer"+me){
    ...
    while(gap<n){
      if(me+gap < n) sum(me+gap) = s // pass my value up the line
      barrier.sync()
      if(gap <= me){                // receive from me-gap,
	         val inc = sum(me) // inc = sum a(me-2*gap .. me-gap]
...
class GridMax(n: Int, xss: Array[Array[Int]]){
  ...
  private val maxs = xss

  private def worker(i: Int, j: Int, x: Int, readUp: ?[Int], writeUp: ![Int], readRight: ?[Int], writeRight: ![Int]): PROC = proc{
    var max = x
    if (i==0){
      writeUp!x
    }
  ...
  private val ups, rights = Array.fill(n, n)(OneOneBuf[Int](1))

  private def system: PROC = || (for (i <- 0 until n; j <-  0 until n) yield worker(i, j, xss(i)(j), ups(i)(j), ups((i+1)%n)(j), rights(i)(j), rights(i)((j+1)%n)))
...
class LogGridMax(n: Int, xss: Array[Array[Int]]){
  ...
  // O(log n) rounds by building tree structure both row wise and column wise; share max with horizontal and vertical children.
  private def worker(i: Int, j: Int, x: Int, read: ?[Int], write: List[List[![Int]]]): PROC = proc{
    var max = x

    val vparent = (i-1)/2; val hparent = (j-1)/2 // parents
    val vchild1 = 2*i+1; val vchild2 = 2*i+2; val hchild1 = 2*j+1; val hchild2 = 2*j+2
    ...
    if(hchild1 < n) write(i)(hchild1)!max // pass column max to children
    if(hchild2 < n) write(i)(hchild2)!max

    xss(i)(j) = max
  }
  // The running time is O(log n), assuming there are as many processors as processes as we have to wait for the messages to reach the bottom of the vertical and the horizontal trees.

  private val chans = List.fill[Chan[Int]](n, n)(N2NBuf[Int](size = 1, readers = 1, writers = n-1))
  private def system: PROC = || (for (i <- 0 until n; j <-  0 until n) yield worker(i, j, xss(i)(j), chans(i)(j), chans))
...
class SmoothShared(a: Array[Array[Boolean]], p: Int, maxIters: Int){
  ...
  private def worker(startR: Int, endR: Int): PROC = proc{
    var done = false
    var k = 0
  ...
  private def rowChunk = nRows/p

  private def system = || (for (i <- 0 until p) yield worker(i*rowChunk, (i+1)*rowChunk))
...
class BagOfTasks(a: Double, b: Double, n: Int, nTasks: Int){
  ...
  /* Get a task.  @throws Stopped exception if there are no more tasks. */
  def getTask: Task = synchronized{
    if (i >= nTasks) io.threadcso.stop
...
class TerminatingPartialQueue[A](numWorkers: Int){
  ...
  def enqueue(x: A): Unit = synchronized {
    if (!shut){
      queue.enqueue(x); if (waiting>0) notify()
    }
  }
  ...
  def shutdown = synchronized{ shut = true}
...
class ConcGraphSearch[N](g: Graph[N]) extends GraphSearch[N](g){
  ...
  def apply(start: N, isTarget: N => Boolean): Option[List[N]] = {
    ...
    def foundPath(path: List[N]) = synchronized{
      result = Some(path)
      queue.shutdown
      done = true
    }
    ...
    def worker = proc("worker"){
      while(!done){
        ...
        res match{
          case Some((n, path)) => for(n1 <- g.succs(n)){
            if(isTarget(n1)) foundPath(path :+ n1) // done!
            else if(seen.add(n1)) queue.enqueue((n1, path :+ n1))
          }
          case None => done=true
        }
    ...
    || (for(_ <- 0 until numWorkers) yield worker)()
    result
  }
}
