...
($>) :: M a -> (a -> M b) -> M b
(xm $> f) mem =
  let (cs, x, mem1) = xm mem in let (cs',x',mem2) = f x mem1 in (cs++cs',x',mem2)
...
init_env =
  make_env [constant "nil" Nil, 
    ...
    primitive "print" (\ [IntVal v] -> printint v $> \ _ -> result Nil)]
