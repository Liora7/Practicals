...
let rec repeat code n =
    match n with
      0 -> SEQ[]
    | _ -> SEQ[code; repeat code (n-1)]
...
let gen_addr d =
  if d.d_level = 0 then
    ...
    let n = d.d_level and m = !level in
    SEQ[LOCAL 0; (repeat (SEQ[CONST 12; OFFSET; LOADW]) (m-n)); CONST d.d_off; OFFSET]
...
let rec gen_expr =
  function
    ...
    | Call (p, args) ->
      let n = List.length args and proc = get_def p in
        let l = !level and m = proc.d_level - 1 in
        match m with
          0 -> SEQ [LINE p.x_line; SEQ(List.rev_map gen_expr args);
               CONST 0; GLOBAL proc.d_lab; PCALLW n]
          (*| _ -> failwith (string_of_int (m-1))*)
          | _ -> SEQ [LINE p.x_line; SEQ(List.rev_map gen_expr args);
                 LOCAL 0; (repeat (SEQ[CONST 12; OFFSET; LOADW]) (l-m)); GLOBAL proc.d_lab; PCALLW n]
