...
let rec gen_stmt s exit_lab =
  match s with
      Skip -> NOP
    | Seq stmts -> SEQ (List.map (fun stmt -> gen_stmt stmt exit_lab) stmts)
    | Assign (v, e) ->
...
let translate (Program ss) =
  let exit_lab = label () in
    let code = gen_stmt ss exit_lab in
    Keiko.output (if !optflag then Peepopt.optimise (SEQ[code; LABEL exit_lab]) else  SEQ[code; LABEL exit_lab])
