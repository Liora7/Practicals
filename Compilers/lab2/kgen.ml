...
let rec gen_expr e =
  ...
  match e.e_guts with
      Variable _ | Sub _ ->
        if (type_size t = 1) then SEQ [gen_addr e; LOADC] else
...
and gen_addr v =
  match v.e_guts with
    ...
    | Sub (x, i) ->
        let t_size = type_size (base_type x.e_type) in
          SEQ [gen_addr x; gen_expr i; CONST t_size; BINOP Times; OFFSET ]
    | _ ->
        failwith "gen_addr"
...
let rec gen_stmt =
  function
    ...
    | Assign (v, e) ->
        if (type_size v.e_type = 1) then SEQ [LINE (line_number v); gen_expr e; gen_addr v; STOREC] else
