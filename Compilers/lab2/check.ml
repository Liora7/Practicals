...
and expr_type e env =
  match e.e_guts with
      Variable x ->
        lookup_def x env
    | Sub (v, e2) ->
        let t1 = check_expr v env and t2 = check_expr e2 env in
          if (is_array t1 && t2 = Integer) then (base_type t1) else failwith "not valid subscript expression"
    | Constant (n, t) -> t
    | Monop (w, e1) ->
...
let rec check_stmt s env =
  match s with
    ...
    | Assign (lhs, rhs) ->
        ...
        if (ta <> tb && not (is_array ta)) then sem_error "type mismatch in assignment" []
    | Print e ->
