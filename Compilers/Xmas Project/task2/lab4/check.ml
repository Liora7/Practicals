...
(* Association list to associate procedures to lists of variables accessed*)
let proc_vars = ref []
...
(* |check_var| -- check that expression denotes a variable and return it *)
        ...
        begin
          match d.d_kind with
              VarDef | VParamDef | CParamDef ->
                d.d_mem <- d.d_mem || addressible; [x.x_name]
            | _ ->
...
(* |check_expr| -- check and annotate an expression, return its type and variables used *)
let rec check_expr e env =
  let (t, l) = expr_type e env in
  e.e_type <- t; (t, l)

(* |expr_type| -- return type of expression and variables used *)
and expr_type e env =
  match e.e_guts with
      Variable x ->
    ...
    | FuncCall (p, args) ->
        let v = ref None in
        let (t1, l1) = check_funcall p args env v in
        if same_type t1 voidtype then
          sem_error "$ does not return a result" [fId p.x_name];
        e.e_value <- !v; (t1, l1)
    | Monop (w, e1) ->
        let (t1, l1) = check_expr e1 env in
        let t = check_monop w t1 in
        e.e_value <- try_monop w e1.e_value;
        (t, l1)
    | Binop (w, e1, e2) ->
        let (t1, l1) = check_expr e1 env in
        let (t2, l2) = check_expr e2 env in
        let t = check_binop w t1 t2 in
        e.e_value <- try_binop w e1.e_value e2.e_value;
        (t, List.append l1 l2)

(* |check_funcall| -- check a function or procedure call and return variables used *)
and check_funcall f args env v =
  let d = lookup_def f env in
  match d.d_kind with
      LibDef q ->
        let l = check_libcall q args env v in (d.d_type, l)
    | ProcDef | PParamDef ->
        let p = get_proc d.d_type in
        let l1 = check_args p.p_fparams args env in
        let l2 = try List.assoc d.d_tag !proc_vars
        with Not_found -> [] in
        (p.p_result, List.append l2 l1)
    | _ -> sem_error "$ is not a procedure" [fId f.x_name]

(* |check_args| -- match formal and actual parameters and return list of variables accessed *)
and check_args formals args env =
  let ls = try List.map2 (fun f a -> check_arg f a env) formals args with
    Invalid_argument _ ->
      sem_error "wrong number of arguments" [] in
  List.concat ls

(* |check_arg| -- check one (formal, actual) parameter pair and return variables if there are any *)
and check_arg formal arg env =
  match formal.d_kind with
      CParamDef | VParamDef ->
        let (t1, l1) = check_expr arg env in
        if not (same_type formal.d_type t1) then
          sem_error "argument has wrong type" [];
        if formal.d_kind = VParamDef then
          let l = check_var arg true in List.append l l1
        else l1
    | PParamDef ->
...
(* |check_libcall| -- check call to a library procedure and return list of variables*)
and check_libcall q args env v =
  ...
  if q.q_argtypes <> [] then
    let check t e =
      let (t1, l1) = check_expr e env in
      if not (same_type t t1) then
        sem_error "argument of $ has wrong type" [fLibId q.q_id]
      else l1 in
    let ls = List.map2 check q.q_argtypes args in
    List.concat ls
  else
  match q.q_id with
      ChrFun ->
    ...
    | OrdFun ->
        ...
        v := e1.e_value; l1
    | PrintString ->
        let (t1, l1) = check_expr (List.hd args) env in
        if not (is_string t1) then
          sem_error "print_string expects a string" [];
        l1
    | ReadChar ->
        let l = check_var (List.hd args) true in l
    | NewProc ->
        let (t1, l1) = check_expr (List.hd args) env in
        if not (is_pointer t1) then
          sem_error "parameter of new must be a pointer" [];
        let l = check_var (List.hd args) false in List.append l1 l
    | ArgvProc ->
        let (t1, l1) = check_expr (List.nth args 0) env
        and (t2, l2) = check_expr (List.nth args 1) env in
        if not (same_type t1 integer) || not (is_string t2) then
          sem_error "type error in parameters of argv" [];
        let l = check_var (List.nth args 1) true in List.append l (List.append l1 l2)
    | OpenIn ->
        let (t1, l1) = check_expr (List.nth args 0) env in
        if not (is_string t1) then
...
(* |check_stmt| -- check and annotate a statement, return list of variables accessed *)
let rec check_stmt s env alloc =
  ...
  match s.s_guts with
    ...
    | Seq ss ->
        List.fold_right (fun s1 -> List.append (check_stmt s1 env alloc) ) ss []

    | Assign (lhs, rhs) ->
        let (lt, lv) = check_expr lhs env
        and (rt, rv) = check_expr rhs env in
        let v = check_var lhs false in
        if not (same_type lt rt) then
          sem_error "type mismatch in assignment" []
        else
        List.append v (List.append lv rv);

    | ProcCall (p, args) ->
        let (rt, l) = check_funcall p args env (ref None) in
        if rt <> voidtype then
    ...
    | IfStmt (cond, thenpt, elsept) ->
        let (ct, l) = check_expr cond env in
        if not (same_type ct boolean) then
          sem_error "test in if statement must be a boolean" [];
        let l1 = check_stmt thenpt env alloc in
        let l2 = check_stmt elsept env alloc in
        List.append l (List.append l1 l2)

    | WhileStmt (cond, body) ->
        let (ct, l) = check_expr cond env in
        if not (same_type ct boolean) then
          sem_error "type mismatch in while statement" [];
        let l1 = check_stmt body env alloc in
        List.append l l1

    | RepeatStmt (body, test) ->
        let l = check_stmt body env alloc in
        let (ct, l1) = check_expr test env in
        if not (same_type ct boolean) then
    ...
    | ForStmt (var, lo, hi, body, upb) ->
        let (vt, l1) = check_expr var env in
        let (lot, l2) = check_expr lo env in
        let (hit, l3) = check_expr hi env in
        ...
        let v = check_var var false in
        let l4 = check_stmt body env alloc in
        ...
        alloc d; upb := Some d;
        List.append l1 (List.append l2 (List.append l3 (List. append v l4)))

    | CaseStmt (sel, arms, deflt) ->
        let (st, l1) = check_expr sel env in
        ...
        let check_arm (lab, body) =
          ...
          let l = check_stmt body env alloc in (v, l) in

        let (vs, ls) = List.split (List.map check_arm arms) in
        check_dupcases vs;
        let l2 = check_stmt deflt env alloc in
        List.append l1 (List.append (List.concat ls) l2)

    | CommStmt (stms1, stms2) ->
       let vars1 = check_stmt stms1 env alloc in
       let vars2 = check_stmt stms2 env alloc in
       let rec compare l1 l2 =
        match l1 with
          [] -> []
        | e :: es -> if (List.exists (fun x -> x = e) l2) then (e :: compare es l2) else (compare es l2) in
       let shared = compare vars1 vars2 in
       if (List.length shared > 0) then Printf.eprintf "Possibly incorrect \n" else Printf.eprintf "Correct \n"; exit 0
...
(* |check_block| -- check a local block and return list of variables*)
let rec check_block level rt env (Block (ds, ss, fsize, nregv)) =
  ...
  check_bodies env' ds;
  return_type := rt;
  let l = check_stmt ss env' pre_alloc in
  ...
  let check =
    function
        ProcDecl (Heading(x, _, _), body) ->
          ...
          incr level;
          let l = check_block d.d_level p.p_result env' body in
          decr level;
          proc_vars := (d.d_tag, l) :: !proc_vars; ()
      | _ -> () in
  let ls = List.map check ds in ()
