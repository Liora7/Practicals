  ...
  let code =
    match s.s_guts with
      ...
      | ForStmt (var, elem, body, t) ->
          (* Use previously allocated temp variable to store upper bound or index.
             We could avoid this if the upper bound is constant. *)

          let gen_elem el =
            match el with
              Range (lo, hi) ->
                let tmp = match !t with Some d -> d | _ -> failwith "for" in
                let l1 = label () and l2 = label () in
                <SEQ,
                  ...
                  <LABEL l2>>
            | Elems (v, length, size) ->
                if (!length  = 0) then <NOP> else
                let tmp = match !t with Some d -> d | _ -> failwith "for" in
                let l1 = label () and l2 = label () in
                <SEQ,
                  <STOREW, <CONST 0>, address tmp>,
                  <LABEL l1>,
                  <JUMPC (Gt, l2), <LOADW, address tmp>, <CONST (!length - 1)>>,
                  <STOREW,
                    <LOADW,
                      <OFFSET,
                        gen_addr v,
