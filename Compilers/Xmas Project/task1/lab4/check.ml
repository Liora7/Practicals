          ...
          function
            Range (lo, hi) ->
              ...
              if not (same_type vt integer) || not (same_type lot integer)
                ...
                sem_error "type mismatch in for range" [];
              check_var var false;
              check_stmt body env alloc;
          | Elems (v, length, size) ->
              (match v.e_guts with
                Variable n ->
                  let vt = check_expr v env in
                  let arr_def = lookup n.x_name env in
                  let a_type = arr_def.d_type in
                  let arr_size = bound a_type in
                  let arr_type = base_type a_type in
                  check_var v false;
                  let var_name = (match var.e_guts with Variable nm -> nm) in
                  let var_def = make_def var_name.x_name VarDef arr_type in
                  var_def.d_addr <- Global (sprintf "_$" [fId var_def.d_tag]);
                  var_name.x_def <- Some var_def;
                  let env' = add_block [var_def] env in
                  incr level;
                  let vts = check_expr var env' in
                  check_var var false;
                  check_stmt body env' alloc;
                  decr level;
                  length := arr_size; size := arr_type.t_rep.r_size;
                | _ -> sem_error "type mismatch in for loop" []; ) in


              check_elem elem;

              (* Allocate space for hidden variable.  In the code, this will
                 be used to save the upper bound or temporary element.  *)
