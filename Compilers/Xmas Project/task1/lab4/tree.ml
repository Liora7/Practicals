...
and stmt_guts =
  ...
  | ForStmt of expr * for_elem * stmt * def option ref
  | CaseStmt of expr * (expr * stmt) list * stmt

and for_elem =
    Range of expr * expr
  | Elems of expr * int ref * int ref
...
and fStmt s =
  match s.s_guts with
    ...
    | ForStmt (var, elem, body, _) ->
        fMeta "(FOR $ $ $)" [fExpr var; fElem elem; fStmt body]
